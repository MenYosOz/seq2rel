...
local max_steps = std.parseInt(std.extVar('max_steps'));         // Max number of decoding steps
local reinit_layers = std.parseInt(std.extVar('reinit_layers'));      // Re-initializes the last N layers of the encoder
...
local validation_start = std.max(std.floor(num_epochs - 10), 0);
